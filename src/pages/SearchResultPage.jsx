...
//API key
  ...
  const url = base_url+"games?search="+search+"&key="+apiKey
  ...
  const FetchSearch = async () => {
    try{
        ...
        setLoading(false)
        console.log('searchhhhhii gamess', data.results)
        console.log('searchh fetched',searchs )
        return data
    }catch(err){
        console.log(err)
        return err
  ...
  return (
    <>{loading == false && searchs != '' ?
      
      
    <Countainer>
      <Title>Results for: {search}</Title>
      <GamesList>
        {searchs ? searchs.map((item)=>(
